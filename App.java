...
public class App {
    ...
    // Nome do arquivo CSV para salvar tarefas
    static final String CSV = "data_tasks.csv";
    // Número máximo de tarefas
    static final int MAX = 5000;

    // Arrays que armazenam as tarefas em memória
    ...
    static int n = 0;                       // Número de tarefas ativas

    public static void main(String[] args) throws Exception {
        carregar(); // Carrega as tarefas do CSV

        // Cria servidor HTTP
        HttpServer server = HttpServer.create(new InetSocketAddress(PORT), 0);
        server.createContext("/", new RootHandler());         // Contexto para página principal
        server.createContext("/api/tasks", new ApiTasksHandler()); // Contexto para API REST
    ...
    static class RootHandler implements HttpHandler {
        @Override public void handle(HttpExchange ex) throws IOException {
            if (!"GET".equalsIgnoreCase(ex.getRequestMethod())) {
                send(ex, 405, ""); // Método não permitido
                return;
            }
        ...
        @Override public void handle(HttpExchange ex) throws IOException {
            ...
            try {
                ...
                if ("DELETE".equals(method) && path.startsWith("/api/tasks/")) {
                    ...
                    int i = findIdxById(id);
                    if (i < 0) { sendJson(ex, 404, "{\"error\":\"not found\"}"); return; }
                    // Remove tarefa e desloca arrays
                    for (int k = i; k < n - 1; k++) {
                        ids[k] = ids[k+1];
                ...
                send(ex, 404, ""); // Endpoint não encontrado
            } catch (Exception e) {
                e.printStackTrace();
...
// Cria um elemento HTML a partir de string
function el(html){
...
// Renderiza todas as tarefas nas colunas
function render(arr){
...
// Funções para atualizar status e excluir
    ...
    static void carregar() {
        ...
        try (BufferedReader br = Files.newBufferedReader(p, StandardCharsets.UTF_8)) {
            ...
            while ((line = br.readLine()) != null) {
                if (line.isBlank() || line.startsWith("id;")) continue; // Ignora cabeçalho ou linhas vazias
...
// ------------------ Funções utilitárias ------------------
    ...
    // Pega valor de chave em JSON simples
